...
- Deepen customer insights                    
    ...
    if selection == "Key Metrics":
        ...
        Total_cost= final_df['cogs_usd_orderitem'].sum()/1000
        Net_cost=final_df[final_df['order_item_refund_id'].isna()]['cogs_usd_orderitem'].sum()/1000
        Profit = ((final_df['price_usd_orderitem'].sum() - final_df['refund_amount_usd'].sum()) - final_df[final_df['order_item_refund_id'].isna()]['cogs_usd_orderitem'].sum()) / 1000000
        Profit_percent = ((final_df['price_usd_orderitem'].sum()- final_df['refund_amount_usd'].sum()- final_df[final_df['order_item_refund_id'].isna()]['cogs_usd_orderitem'].sum())/ final_df[final_df['order_item_refund_id'].isna()]['cogs_usd_orderitem'].sum()) * 100
       ...
       .assign(CVR_percentage=lambda x: round((x["conversions"] / x["total_sessions"]) * 100, 2)).reset_index())
        ...
        traffic_pivot = (traffic_time_pattern.pivot(index="hour", columns="weekday", values="session_count").reindex(columns=weekday_order)  .fillna(0))
